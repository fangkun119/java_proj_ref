...
public class CNPTrxDemoProducer {
    // 设为true时会触发事务终止（默认为false）
    boolean isTriggerTrxAbort = false;
    public CNPTrxDemoProducer setTriggerTrxAbort(boolean isTriggerTrxAbort) {
        this.isTriggerTrxAbort = isTriggerTrxAbort;
        return this;
    };
    ...
    public void runDemo(String bootstrapServers) {
        ...
        try {
            ...
            for (int i = 0; i < 30; ++i) {
                if (isTriggerTrxAbort && 8 == i) {
                    int j = 10 / 0; // 如果设置了isTriggerTrxAbort开关，在发送第8条数据的时候引发一个异常，构造事务终止的场景
                }
